...
export class ClientAccessPolicy {
  ...
  async canViewClient(
    ...
    if (['super_admin', 'admin', 'diretoria', 'gerencia'].includes(role)) {
      return true;
    }
  ...
  async canAssignManagerOrTeam(role: string): Promise<boolean> {
    return ['super_admin', 'admin', 'diretoria', 'gerencia'].includes(role);
  }

  // 5. Arquivar/churn: diretoria/administração
  async canArchiveClient(role: string): Promise<boolean> {
    return ['super_admin', 'admin', 'diretoria'].includes(role);
  }
