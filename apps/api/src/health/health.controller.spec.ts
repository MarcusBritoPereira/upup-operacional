...
describe('HealthController', () => {
  it('reports liveness without accessing the database', () => {
    const { controller, prisma } = createController(Promise.resolve([{ '?column?': 1 }]));

    expect(controller.liveness()).toMatchObject({ status: 'ok', check: 'live' });
    expect(prisma.$queryRaw).not.toHaveBeenCalled();
  });

  it('reports readiness when the database responds', async () => {
    const { controller } = createController(Promise.resolve([{ '?column?': 1 }]));
  ...
  it('returns service unavailable when the database is down', async () => {
    const { controller } = createController(Promise.reject(new Error('offline')));
