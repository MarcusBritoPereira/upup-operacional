...
export default function ProvidersPage() {
  ...
  if (loading) return (
    <DashboardLayout>
      <div className="p-8 text-center text-[var(--muted-foreground)]">Carregando prestadores...</div>
    </DashboardLayout>
  );
  if (error) return (
    <DashboardLayout>
    ...
    <DashboardLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[var(--card)] p-6 rounded-xl border border-[var(--border)] shadow-sm">
        <div>
          ...
          <p className="text-[var(--muted-foreground)] text-sm mt-1">Gerencie a rede de freelancers e parceiros</p>
        </div>
        <button
        ...
        {providers.map((provider) => (
          <div key={provider.id} className="bg-[var(--card)] p-5 rounded-xl shadow-xs border border-[var(--border)] hover:border-[var(--border)] transition">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => handleDelete(provider.id)} className="text-[var(--muted-foreground)] hover:text-rose-500 p-1">
            ...
            <div className="mt-4 space-y-2 text-sm text-[var(--muted-foreground)]">
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-[var(--muted-foreground)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                {provider.email || <span className="text-[var(--muted-foreground)] italic">Não informado</span>}
              </div>
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-[var(--muted-foreground)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                {provider.whatsapp || <span className="text-[var(--muted-foreground)] italic">Não informado</span>}
              </div>
            </div>

            <div className="mt-4 pt-4 border-t border-[var(--border)]">
              <details className="group">
                <summary className="text-sm font-medium text-[var(--muted-foreground)] cursor-pointer hover:text-[var(--foreground)] transition flex items-center gap-1 select-none outline-none">
                ...
                <div className="mt-3 pl-5 space-y-2">
                  {[...(provider.clientLinks || [])]
                    ...
                    return (
                      <div key={link.id} className="text-sm text-[var(--muted-foreground)] flex items-center gap-2">
                        ...
                        {isRepeated && link.role && (
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-[var(--border)] text-[var(--muted-foreground)]">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--card)] rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-[var(--muted-foreground)] hover:text-[var(--muted-foreground)] transition">
