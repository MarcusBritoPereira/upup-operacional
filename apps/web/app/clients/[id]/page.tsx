    ...
    <DashboardLayout>
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="border-b border-[var(--border)] flex gap-6 overflow-x-auto">
          ...
          {(user?.role === 'super_admin' || user?.role === 'admin' || user?.role === 'diretoria') && (
            <button
        ...
        {activeTab === 'senhas' && (user?.role === 'super_admin' || user?.role === 'admin' || user?.role === 'diretoria') && (
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <select value={managerId} onChange={(e) => setManagerId(e.target.value)} className="form-input">
                              <option value="">Selecione</option>
                              {users.filter(u => u.role === 'gestor_cliente' || u.role === 'admin' || u.role === 'super_admin').map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                            </select>
                          </div>
                        </div>
                      </div>
